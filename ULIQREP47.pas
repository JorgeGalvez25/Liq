...
procedure TFLIQREP47.CreaEtiquetas;
var i,xl,xw:integer;
begin
  xl:=xLeft;
  for i:=1 to 10 do begin
    ...
    with QRLabelEnca[i] do begin
      ...
      if i<10 then Frame.DrawRight:=true;
      Frame.Width:=1;
    end;
...
procedure TFLIQREP47.IniciaEtiquetasCmb;
var i:word;
begin
  xl:=xLeft;
  for i:=1 to 10{8} do begin
    with QRLabelEnca[i] do begin
      Case i of
        ...
        4:Caption:='Importe';
        5:Caption:='Descuento';
        6:Caption:='ImporteNeto';
        7:Caption:='SubTotal';
        8:Caption:='Costo Unit.';
        9:Caption:='Utilidad';
        10:Caption:='%';
      end;
      Case i of
...
procedure TFLIQREP47.IniciaEtiquetasProd;
var i:word;
begin
  xl:=xleft;
...
procedure TFLIQREP47.PreparaReporte;
var TotCmb,TotAlma,TotPago:Array[1..9] of real;
    i:word;
    Precio,SubTotal,
    Costo,PorcUtil,Utilidad,
    Cantidad,Importe,ImporteNeto,Descuento,
    Iva,Ajuste,Pago,
...
begin
  with DMGEN,DMCUP,DMLIQ,DMGAS do begin
    ...
    while not QL_Dliqc.Eof do begin
      ...
      Q_Auxi.SQL.Clear;
      Q_Auxi.SQL.Add('Select sum(Cantidad) As Real1, sum(Importe) As Real2, Sum(Importe_IEPS) As Real3 ');
      Q_Auxi.SQL.Add('From DGASDLIQC');
      ...
      DMCUP.Q_Auxi.Close;
      DMCUP.Q_AuxiReal1.FieldKind:=fkInternalCalc;
      DMCUP.Q_Auxi.SQL.Clear;
      DMCUP.Q_Auxi.SQL.Add('Select sum(Descuento) As Real1 ');
      DMCUP.Q_Auxi.SQL.Add('From DGASDESCD');
      DMCUP.Q_Auxi.SQL.Add('Where Estacion='+IntToStr(pEstacion));
      DMCUP.Q_Auxi.SQL.Add('  and Combustible='+IntToStr(QL_DliqcCombustible.AsInteger));
      DMCUP.Q_Auxi.SQL.Add('  and Fecha="'+FechaToStrSQL(pFecha)+'"');
      DMCUP.Q_Auxi.SQL.Add('  and Turno='+IntToStr(pTurno));
      DMCUP.Q_Auxi.Prepare;
      DMCUP.Q_Auxi.Open;

      Cantidad:=Q_AuxiReal1.AsFloat;
      Importe:=Q_AuxiReal2.AsFloat;
      Descuento:=DMCUP.Q_AuxiReal1.AsFloat;
...
//        end;
      ...
      SubTotal:=AjustaFloat(DivideFloat(ImporteNeto,1+PrecTasaIva/100),2);
      Iva:=ImporteNeto-SubTotal;
      ...
      if DMCUP.SwSubModuloCupReformas2008 then begin
        ...
        IEPS:=cantidad*IEPS;
        ImporteAIEPS:=(ImporteNeto-IEPS)/(1+(PrecTasaIva/100));
        Iva:=(ImporteNeto-IEPS-ImporteAIEPS);
      ...
      TotCmb[4]:=TotCmb[4]+Descuento;
      TotCmb[5]:=TotCmb[5]+ImporteNeto;
      TotCmb[6]:=TotCmb[6]+SubTotal;
      TotCmb[7]:=TotCmb[7]+Costo;
      TotCmb[8]:=TotCmb[8]+Utilidad;
      QL_Dliqc.Next;
    end;
    TotCmb[9]:=AjustaFloat(DivideFloat(TotCmb[8],TotCmb[6])*100,2);
    xlin:='T// //';
    for i:=1 to 9 do begin
      Case i of
        1,3..8:xlin:=xlin+FormatoMoneda(TotCmb[i])+'//';
        2:xlin:=xlin+'//';
        9:xlin:=xlin+FormatFloat('##0.00',TotCmb[9]);
      end;
    end;
    BndDt.Items.Add(xlin);
...
// Aceites y Derivados
    ...
    BndDt.Items.Add('');
//
    SubTotal:=TotCmb[5]+TotAlma[3];
    BndDt.Items.Add('S//Resumen de Ventas:');
    BndDt.Items.Add('D//Combustibles// //'+FormatoMoneda(TotCmb[5])+'//'+FormatFloat('##0.00',AjustaFloat(DivideFloat(TotCmb[5],SubTotal)*100,2))+'%');
    ...
    BndDt.Items.Add('R// // //'+FormatoMoneda(TotCmb[5]+TotAlma[3])+'//'+FormatFloat('#00.00',100.00)+'%');
//
    ...
    while not QL_Dliqp.Eof do begin
      ...
      if (QL_DliqpClasePago.AsString='Efectivo') and (not swTPagEfectivoConDescuento) then begin
        Pago:=Pago-TotCmb[4]; //Se le resta el descuento global (combustibles)
...
procedure TFLIQREP47.QuickRep1BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
...
procedure TFLIQREP47.BndDtBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
  ...
  begin
    for k:=ri to rf do
      with QRLabelDeta[k] do begin
        ...
        if xtp='T' then begin
          Font.Style:=[fsBold];
          if k in [2,4..10{7}] then Frame.DrawTop:=true
          else Frame.DrawTop:=false;
        end;
        if xtp='U' then begin
          Font.Style:=[fsBold];
          if k in [2,4..8{7}] then Frame.DrawTop:=true
          else Frame.DrawTop:=false;
        end;
        if xtp='Y' then begin
          Font.Style:=[fsBold];
...
begin
  ...
  InicializaEtiquetas(1,10{8});
  ExtraeEtiquetas(1,10{8});
end;

procedure TFLIQREP47.ChildBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  PrintBand:=xtp='T';
end;

procedure TFLIQREP47.CreaTablaTemp;
begin
  with DMGEN do begin
...
procedure TFLIQREP47.TT_AuxiCalcFields(DataSet: TDataSet);
begin
