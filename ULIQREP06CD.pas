...
uses
  ...
  Dialogs, DBTables, QRExport, DB, QuickRpt, QRCtrls, ExtCtrls, Master_Intf,
  Printers;

type
  TFLIQREP06CD = class(TForm)
    QuickRep1: TQuickRep;
    QRBand1: TQRBand;
    QRLEmp: TQRLabel;
    QRLDir: TQRLabel;
...
uses ULIQREP06P, DDMGAS, DDMGEN, UAVANCE, DDMCUP, DDMLIQ, DDM_KIOS,
     ULIBGRAL, ULIBDATABASE, DDMGASQ;

{$R *.dfm}

procedure TFLIQREP06CD.CreaEtiquetas;
var i,xl,xw:integer;
begin
  xw:=xAncho;
  xl:=xLeft;
...
procedure TFLIQREP06CD.PreparaReporte;
var tot:Array[3..14] of real;
    tota:Array[3..10] of real;
    totDesc:Array[3..14] of real;
    totaDesc:Array[3..10] of real;
    i,xind,j:word;
    xfecha:TDateTime;
  ...
  with DMGEN,DMLIQ,DMGAS,DM_KIOSK do begin
    ...
    try
      ...
      while not QL_Dliqc.Eof do begin
        ...
        totlDesc:=totaDesc[3]+totaDesc[7];
        totsDesc:=totaDesc[4]+totaDesc[8];
        totiDesc:=totaDesc[5]+totaDesc[9];
        tottDesc:=totaDesc[6]+totaDesc[10];
        totDesc[11]:=totDesc[11]+totlDesc;
        totDesc[12]:=totDesc[12]+totsDesc;
        totDesc[13]:=totDesc[13]+totiDesc;
        totDesc[14]:=totDesc[14]+tottDesc;
...
procedure TFLIQREP06CD.QuickRep1BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
...
procedure TFLIQREP06CD.BndDtBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
    ...
    for k:=ri to rf do
      with QRLabelDeta[k] do begin
        ...
        if xtp='TD' then begin
          Font.Style:=[fsBold];
          if (k>1) and not (k in [2,6,10])then begin
            Frame.DrawTop:=true;
          end;
        end;
      end;
...
procedure TFLIQREP06CD.QRChildBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  PrintBand:=xtp='T';
end;

procedure TFLIQREP06CD.QL_DliqcCalcFields(DataSet: TDataSet);
begin
  with DMGAS do begin
