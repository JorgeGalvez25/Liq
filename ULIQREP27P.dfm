object FLIQREP27P: TFLIQREP27P
  Left = 233
  Top = 199
  BorderStyle = bsDialog
  Caption = 'Relaci'#243'n de Vales por Despachador'
  ClientHeight = 216
  ClientWidth = 395
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = True
  Position = poScreenCenter
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel2: TPanel
    Left = 0
    Top = 164
    Width = 395
    Height = 52
    Align = alBottom
    BevelOuter = bvNone
    TabOrder = 2
    object BPrever: TBitBtn
      Left = 20
      Top = 11
      Width = 76
      Height = 28
      Caption = '&Prever'
      Default = True
      TabOrder = 0
      OnClick = BImprimirClick
      Glyph.Data = {
        
        A4A4AFFCE68CA4A4A4A4A4C7DDD9DBDBDBDBDDE689A4A4A4A4A4}
    end
    object BImprimir: TBitBtn
      Left = 112
      Top = 11
      Width = 76
      Height = 28
      Caption = '&Imprimir'
      TabOrder = 1
      OnClick = BImprimirClick
      Glyph.Data = {
        
        D1FFACEDE5E5E5E5E5E5E5EDF2F5F5FAFAFDEFE5E5E5E5E5E5E5}
    end
    object BOpciones: TBitBtn
      Left = 206
      Top = 11
      Width = 76
      Height = 28
      Caption = '&Opciones'
      TabOrder = 2
      OnClick = BOpcionesClick
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    end
    object BCancelar: TBitBtn
      Left = 298
      Top = 11
      Width = 76
      Height = 28
      Caption = '&Salir'
      TabOrder = 3
      OnClick = BCancelarClick
      Glyph.Data = {
        
        2020202020080808404030202000A7A7A7FFFFFFFFFFFFFFFFFF}
    end
  end
  object GroupBox1: TGroupBox
    Left = 0
    Top = 0
    Width = 233
    Height = 102
    Align = alLeft
    Caption = 'Estaci'#243'n '
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    object DBLookupComboBox2: TDBLookupComboBox
      Left = 8
      Top = 15
      Width = 201
      Height = 21
      Color = clInfoBk
      KeyField = 'CLAVE'
      ListField = 'Descripcion'
      ListSource = DMGAS.DS_Ests
      TabOrder = 0
      OnClick = DBLookupComboBox2Click
      OnEnter = DBLookupComboBox2Enter
    end
    object DBLookupComboBox1: TDBLookupComboBox
      Left = 8
      Top = 48
      Width = 201
      Height = 21
      Color = clInfoBk
      Enabled = False
      KeyField = 'CLAVE'
      ListField = 'ClaveNombre'
      ListSource = DSQL_Desp
      TabOrder = 1
    end
    object CheckBox1: TCheckBox
      Left = 9
      Top = 72
      Width = 184
      Height = 17
      Caption = 'Todos los Despachadores'
      Checked = True
      State = cbChecked
      TabOrder = 2
      OnClick = CheckBox1Click
    end
  end
  object GroupBox2: TGroupBox
    Left = 233
    Top = 0
    Width = 162
    Height = 102
    Align = alClient
    Caption = 'Per'#237'odo '
    TabOrder = 1
    object Label1: TLabel
      Left = 14
      Top = 25
      Width = 58
      Height = 13
      Alignment = taRightJustify
      Caption = 'De la fecha:'
    end
    object Label2: TLabel
      Left = 21
      Top = 52
      Width = 51
      Height = 13
      Alignment = taRightJustify
      Caption = 'A la fecha:'
    end
    object MaskEdit3: TMaskEdit
      Left = 81
      Top = 19
      Width = 68
      Height = 21
      Color = clInfoBk
      EditMask = '!99/99/0000;1;_'
      MaxLength = 10
      TabOrder = 0
      Text = '  /  /    '
      OnExit = MaskEdit3Exit
    end
    object MaskEdit4: TMaskEdit
      Left = 81
      Top = 48
      Width = 68
      Height = 21
      Color = clInfoBk
      EditMask = '!99/99/0000;1;_'
      MaxLength = 10
      TabOrder = 1
      Text = '  /  /    '
    end
  end
  object GroupBox3: TGroupBox
    Left = 0
    Top = 102
    Width = 395
    Height = 62
    Align = alBottom
    Caption = 'Impresora  '
    TabOrder = 3
    object ComboBox1: TComboBox
      Left = 81
      Top = 20
      Width = 233
      Height = 21
      Color = clInfoBk
      ItemHeight = 13
      TabOrder = 0
      Text = 'ComboBox1'
      OnChange = ComboBox1Change
    end
  end
  object PrintDialog1: TPrintDialog
    Left = 320
    Top = 232
  end
  object QL_Desp2: TQuery
    OnCalcFields = QL_Desp2CalcFields
    DatabaseName = 'DBGASOL1'
    SQL.Strings = (
      'Select Clave,Nombre from DGASDESP'
      'Where Estacion=:pestacion'
      'order by Nombre')
    Left = 40
    Top = 124
    ParamData = <
      item
        DataType = ftInteger
        Name = 'pestacion'
        ParamType = ptUnknown
      end>
    object QL_Desp2CLAVE: TIntegerField
      FieldName = 'CLAVE'
    end
    object QL_Desp2NOMBRE: TStringField
      FieldName = 'NOMBRE'
      Size = 40
    end
    object QL_Desp2ClaveNombre: TStringField
      FieldKind = fkCalculated
      FieldName = 'ClaveNombre'
      Size = 40
      Calculated = True
    end
  end
  object DSQL_Desp: TDataSource
    DataSet = QL_Desp2
    Left = 8
    Top = 124
  end
end
