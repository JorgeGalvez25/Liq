...
type
  ...
  public
    ...
    Function ValidaSalida:Boolean;
  end;

var
  FLIQSALISLAN :TFLIQSALISLAN;
...
Procedure TFLIQSALISLAN.EliminaCaptura;
begin
...
procedure TFLIQSALISLAN.PreparaForma(xtipo:string;xmodo:integer);
begin
  with DMINV,DMGEN,DMGAS,DMGEN_VTAS,FLIQTURC do begin
    ...
    try
      ...
      DBLookUpComboBox3.KeyValue:=T_IslaISLA.AsInteger;
      SetAltaDocu;
    except
      EliminaCaptura;
      raise;
...
Procedure TFLIQSALISLAN.SetAltaDocu;
begin
  with DMGEN,DMINV,DMGEN_VTAS,DMGAS,FLIQTURC do begin
    ...
    try
      try
        ...
        T_SaliConcepto1.AsString:='VENTA DE ACEITES Y DERIVADOS DEL DIA ';
        ...
        FLIQSALISLAN.DBGrid1.DataSource:=nil;
        swgrid:=false;
...
procedure TFLIQSALISLAN.SetModificaDocu;
begin
  with DMINV,DMGEN,DMGEN_VTAS do begin
    ...
    try
      try
        ...
        FLIQSALISLAN.DBGrid1.DataSource:=DS_Movs;
        DBEdit2.ReadOnly:=True;
        DBEdit3.ReadOnly:=true
      except
        on e:Exception do begin
          MensajeInfo('Salida '+T_SaliTipoSal.AsString+'-'+IntToClaveNum(T_SaliFolio.AsInteger,6)+#10#13+e.message);
          raise;
        end;
...
procedure TFLIQSALISLAN.Salir1Click(Sender: TObject);
begin
  Close;
end;

procedure TFLIQSALISLAN.Guardar1Click(Sender: TObject);
begin
  with DMINV,DMGEN,DMGAS,DMGEN_VTAS do begin
    ...
    try
      try
        ...
        with DMGAS, FLIQTURC do begin
          Q_Auxi.Close;
          Q_Auxi.SQL.Clear;
          Q_Auxi.SQL.Add('INSERT INTO DGASTURCSALI');
          Q_Auxi.SQL.Add('VALUES('+IntToStr(QL_TurcEstacion.AsInteger));
          Q_Auxi.SQL.Add(','+QuotedStr(FechaToStrSQL(QL_TurcFecha.AsDateTime)));
          Q_Auxi.SQL.Add(','+IntToStr(QL_TurcCAJA.AsInteger));
          Q_Auxi.SQL.Add(','+IntToStr(QL_TurcTURNO.AsInteger));
          Q_Auxi.SQL.Add(','+IntToStr(DBLookUpComboBox3.KeyValue));
          Q_Auxi.SQL.Add(','+QuotedStr(IntToClaveNum(T_SaliSucursal.AsInteger,2)+T_SaliTipoSal.AsString+IntToClaveNum(T_SaliFolio.AsInteger,6))+')');
          Q_Auxi.ExecSQL;
...
procedure TFLIQSALISLAN.FormShow(Sender: TObject);
begin
...
procedure TFLIQSALISLAN.DBGrid1Enter(Sender: TObject);
begin
  DBEdit6.SetFocus;
end;

procedure TFLIQSALISLAN.GroupBox1Enter(Sender: TObject);
begin
...
procedure TFLIQSALISLAN.FormCloseQuery(Sender: TObject;
...
procedure TFLIQSALISLAN.DBLookupComboBox1Enter(Sender: TObject);
begin
  with DMGEN do
...
procedure TFLIQSALISLAN.DBEdit2DblClick(Sender: TObject);
begin
...
procedure TFLIQSALISLAN.DBEdit2Enter(Sender: TObject);
begin
  if DBEdit2.ReadOnly then
    DBEdit8.SetFocus;
end;

procedure TFLIQSALISLAN.DBEdit5Exit(Sender: TObject);
begin
  AgregaListaStrTemp(DBEdit5.Text);
end;

procedure TFLIQSALISLAN.DBEdit5DblClick(Sender: TObject);
begin
  DBEdit5.Text:=SeleccionaStrTemp(DBEdit5.text);
end;

procedure TFLIQSALISLAN.DBEdit4DblClick(Sender: TObject);
begin
  DBEdit4.Text:=SeleccionaStrTemp(DBEdit4.text);
end;

procedure TFLIQSALISLAN.DBEdit4Exit(Sender: TObject);
begin
  AgregaListaStrTemp(DBEdit4.Text);
end;

procedure TFLIQSALISLAN.DBEdit3Enter(Sender: TObject);
begin
  DBLookUpComboBox1.SetFocus;
end;

procedure TFLIQSALISLAN.BbCancelarClick(Sender: TObject);
begin
  with DMINV,DMGEN,DMGEN_VTAS do begin
    if MensajeConf('¿Desea cancelar salida de almacén?')=mrYes then begin
      try
        EliminaCaptura;
...
procedure TFLIQSALISLAN.GroupBox1Exit(Sender: TObject);
begin
  with DMGEN do begin
    try
      SetModificaDocu;
...
procedure TFLIQSALISLAN.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  with DMGEN do
    QT_Ccos.Close;
end;

procedure TFLIQSALISLAN.DBEdit8DblClick(Sender: TObject);
begin
  with DMGEN,DMINV do begin
...
procedure TFLIQSALISLAN.DBEdit1Exit(Sender: TObject);
begin
  DBEdit6.SetFocus;
end;

procedure TFLIQSALISLAN.Edit1Enter(Sender: TObject);
begin
  DBEdit1.SetFocus;
end;

procedure TFLIQSALISLAN.DBEdit9Enter(Sender: TObject);
begin
  DBEdit1.SetFocus;
end;

procedure TFLIQSALISLAN.BbCalcularClick(Sender: TObject);
var xcont      : integer;
    xsaldo     : TSaldoInv;
    cantidad   : double;
...
begin
  with DMINV,DMGAS,FLIQTURC do begin
    if ValidaSalida then begin
      MensajeInfo('Ya existe una salida de almacén en la isla seleccionada');
      Exit;
    end;
    try
      xcont:=0;
...
//Productos
      try
        ...
        while not QL_Prod.Eof do begin
          ...
          Q_Auxi.SQL.Add('  and Isla='+IntToStr(DBLookupComboBox3.KeyValue));
...
procedure TFLIQSALISLAN.FormDestroy(Sender: TObject);
begin
...
Function TFLIQSALISLAN.ValidaSalida:Boolean;
begin
  with DMGAS, FLIQTURC do begin
    Result:=False;
    Q_Auxi.Close;
...
procedure TFLIQSALISLAN.DBEdit2Exit(Sender: TObject);
begin
  with DMINV do begin
    try
      if StrToFloatDef(DBEdit2.Text,0)<1 then begin
        MensajeInfo('Valor inválido');
        DBEdit2.SetFocus;
      end;
      Q_Auxi.Close;
      Q_AuxiEntero1.FieldKind:=fkInternalCalc;
      Q_Auxi.SQL.Clear;
      Q_Auxi.SQL.Add('Select count(*) as Entero1 from DINVSALI');
      Q_Auxi.SQL.Add('where folio='+inttostr(T_SaliFolio.AsInteger));
      Q_Auxi.SQL.Add('and tiposal='+QuotedStr(T_SaliTIPOSAL.AsString));
      Q_Auxi.Prepare;
      Q_Auxi.Open;
      if Q_AuxiEntero1.AsInteger>0 then begin
        MensajeInfo('Folio ya existe');
        DBEdit2.SetFocus;
      end;
    finally
      Q_Auxi.Close;
