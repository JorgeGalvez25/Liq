...
var
  FLIQGRF07P: TFLIQGRF07P;


implementation

uses ULIBGRAL, DDMGEN, UGEN_NET, ULIBDATABASE, ULIBPRINT, DDMGAS,
  ULIQDESGLO, DDMGASP, VarCmplx, DDMCONS, ULIQGRF02, DDMLIQ, ULIQGRF07;

{$R *.DFM}

procedure TFLIQGRF07P.FormShow(Sender: TObject);
var
  xMes:string;
begin
...
procedure TFLIQGRF07P.BCancelarClick(Sender: TObject);
begin
  Close;
end;

procedure TFLIQGRF07P.MaskEdit4KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=idF2 then
...
procedure TFLIQGRF07P.MaskEdit3Exit(Sender: TObject);
begin
  MaskEdit4.Text:=DateToStr(StrToDate(MaskEdit3.Text)+1);
end;

procedure TFLIQGRF07P.BPreverClick(Sender: TObject);
begin
...
procedure TFLIQGRF07P.CBTodasEnter(Sender: TObject);
begin
  if not DMLIQ.ValidaDerechoReportesConsultas(idLiq) then
    MaskEdit3.SetFocus;
end;

procedure TFLIQGRF07P.ActivarDesac(Periodo: byte);
begin
...
procedure TFLIQGRF07P.RGPeriodoClick(Sender: TObject);
begin
...
procedure TFLIQGRF07P.PreparaForma(xModo: Integer;xModuloAuxiliar:Boolean=False);
begin
  swModo:=xModo;
