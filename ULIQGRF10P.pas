...
type
  TFLIQGRF10P = class(TForm)       //Kylie Jenner
    Panel2: TPanel;
    GroupBox2: TGroupBox;
    QL_Tcmb: TQuery;
...
var
  FLIQGRF10P: TFLIQGRF10P;


implementation

uses ULIBGRAL, DDMGEN, UGEN_NET, ULIBDATABASE, ULIBPRINT, DDMGAS,
    VarCmplx, ULIQGRF08, DDMLIQ, UAutoriza, ULIQGRF10;

{$R *.DFM}

procedure TFLIQGRF10P.FormShow(Sender: TObject);
var
  xMes:string;
begin
...
procedure TFLIQGRF10P.BCancelarClick(Sender: TObject);
begin
  Close;
end;

procedure TFLIQGRF10P.MaskEdit4KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=idF2 then
...
procedure TFLIQGRF10P.MaskEdit3Exit(Sender: TObject);
begin
  MaskEdit4.Text:= DateToStr(StrToDate(MaskEdit3.Text)+1);
end;

procedure TFLIQGRF10P.BPreverClick(Sender: TObject);
var
  xanio:string;
begin
...
//    end;
    try
      FLIQGRF10.PreparaForma(swModo,xanio);
      FLIQGRF10.ShowModal;
    finally
      FLIQGRF10.Free;
    end;
...
procedure TFLIQGRF10P.DBLookupComboBox2Enter(Sender: TObject);
begin
...
procedure TFLIQGRF10P.CBTodasEnter(Sender: TObject);
begin
  if not DMLIQ.ValidaDerechoReportesConsultas(idLiq) then
    RGPeriodo.SetFocus;
end;

procedure TFLIQGRF10P.ActivarDesac(Periodo: byte);
begin
...
procedure TFLIQGRF10P.RGPeriodoClick(Sender: TObject);
begin
...
procedure TFLIQGRF10P.PreparaForma(xModo:integer; xModAux:Boolean=False;xModuloAuxiliar:Boolean=False); //25-11-15
begin
