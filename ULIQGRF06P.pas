...
var
  FLIQGRF06P: TFLIQGRF06P;


implementation

uses ULIBGRAL, DDMGEN, UGEN_NET, ULIBDATABASE, ULIBPRINT, DDMGAS,
  ULIQDESGLO, DDMGASP, VarCmplx, DDMCONS, ULIQGRF06, DDMLIQ;

{$R *.DFM}

procedure TFLIQGRF06P.FormShow(Sender: TObject);
var
  xMes:string;
begin
...
procedure TFLIQGRF06P.BCancelarClick(Sender: TObject);
begin
  Close;
end;

procedure TFLIQGRF06P.MaskEdit4KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=idF2 then
...
procedure TFLIQGRF06P.MaskEdit3Exit(Sender: TObject);
begin
  MaskEdit4.Text:=DateToStr(StrToDate(MaskEdit3.Text)+1);
end;

procedure TFLIQGRF06P.BPreverClick(Sender: TObject);
begin
...
procedure TFLIQGRF06P.CBTodasEnter(Sender: TObject);
begin
  if not DMLIQ.ValidaDerechoReportesConsultas(idLiq) then
    MaskEdit3.SetFocus;
end;

procedure TFLIQGRF06P.ActivarDesac(Periodo: byte);
begin
...
procedure TFLIQGRF06P.RGPeriodoClick(Sender: TObject);
begin
...
procedure TFLIQGRF06P.PreparaForma(xModo: Integer;xModuloAuxiliar:Boolean=False);
begin
  swModo:=xModo;
