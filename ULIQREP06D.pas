...
uses
  ...
  Dialogs, QRExport, DBTables, DB, QuickRpt, QRCtrls, ExtCtrls, Master_Intf,
  Printers;

type
  TFLIQREP06D = class(TForm)
    QuickRep1: TQuickRep;
    QRBand1: TQRBand;
    QRLEmp: TQRLabel;
    QRLDir: TQRLabel;
...
uses DDMGAS, DDMGEN, UAVANCE, DDMCUP, DDMLIQ, ULIQREP06P, DDM_KIOS, ULIBGRAL,
     ULIBDATABASE, DDMGASQ;

{$R *.dfm}

procedure TFLIQREP06D.CreaEtiquetas;
var i,xl,xw:integer;
begin
  xw:=xAncho;
  xl:=xLeft;
...
procedure TFLIQREP06D.PreparaReporte;
var tot:Array[3..18] of real;
    tota:Array[3..14] of real;
    totDesc:Array[3..18] of real;
    totaDesc:Array[3..14] of real;    
    i,xind,j:word;
    xfecha:TDateTime;
    totl,tots,toti,totie,tott:real;
    totlDesc,totsDesc,totiDesc,tottDesc:real;
    xtasa,IEPSLts,IEPS,
    xlitros,xsubtotal,xiva,xtotal,ImporteAIEPS:real;
    xsubtotalDesc,xivaDesc,xtotalDesc,ImporteAIEPSDesc:real;
...
begin
  ...
  with DMGEN,DMLIQ,DMGAS,DM_KIOSK do begin
    ...
    QL_Desc.Close;
    QL_Desc.ParamByName('PESTACION').AsInteger:=T_EstsClave.AsInteger;
    QL_Desc.ParamByName('PFECHAINI').AsDateTime:=VgFechaIni;
    QL_Desc.ParamByName('PFECHAFIN').AsDateTime:=VgFechaFin;
    QL_Desc.Prepare;
    QL_Desc.Open;
    ...
    try
      ...
      while not QL_Dliqc.Eof do begin
        ...
        totlDesc:=0; totsDesc:=0; totiDesc:=0; tottDesc:=0;
        for i:=3 to 14 do
          tota[i]:=0;
        for i:=3 to 14 do
          totaDesc[i]:=0;
...
//
          ...
          if LocalizaQuery(QL_Desc,'Fecha;Combustible',VarArrayOf([QL_DliqcFecha.AsDateTime,QL_DliqcCOMBUSTIBLE.AsInteger])) then begin
            xtotalDesc:=QL_DescDESCUENTO.AsFloat;
            xsubtotalDesc:=(xtotalDesc)/(1+GetTasaIVAdeCombustible(T_EstsCLAVE.AsInteger,QL_DliqcCOMBUSTIBLE.AsInteger,QL_DliqcFECHA.AsDateTime)/100);
            xivaDesc:=(xtotalDesc-xsubtotalDesc);
            swDescuento:=True;

            totDesc[xind]:=totDesc[xind];
            totDesc[xind+1]:=totDesc[xind+1]+xSubTotalDesc;
            totDesc[xind+2]:=totDesc[xind+2]+xIvaDesc;
            totDesc[xind+3]:=totDesc[xind+3]+xTotalDesc;

            totaDesc[xind]:=totaDesc[xind];
            totaDesc[xind+1]:=totaDesc[xind+1]+xSubtotalDesc;
            totaDesc[xind+2]:=totaDesc[xind+2]+xIvaDesc;
            totaDesc[xind+3]:=totaDesc[xind+3]+xTotaldesc;
          end;
        ...
        totlDesc:=totaDesc[3]+totaDesc[7]+totaDesc[11];
        totsDesc:=totaDesc[4]+totaDesc[8]+totaDesc[12];
        totiDesc:=totaDesc[5]+totaDesc[9]+totaDesc[13];
        tottDesc:=totaDesc[6]+totaDesc[10]+totaDesc[14];
        totDesc[15]:=totDesc[15]+totlDesc;
        totDesc[16]:=totDesc[16]+totsDesc;
        totDesc[17]:=totDesc[17]+totiDesc;
        totDesc[18]:=totDesc[18]+tottDesc;
      ...
      if swDescuento then begin
        BndDt.Items.Add('DE//Descuentos');
        xlin:='D// //';
        for i:=3 to 18 do
          if i in [3,7,11,15] then xlin:=xlin+' //'
          else xlin:=xlin+FormatoMoneda(totDesc[i])+'//';
        BndDt.Items.Add(xlin);

        xlin:='TD// //';
        for i:=3 to 18 do
          if i in [3,7,11,15] then xlin:=xlin+' //'
          else xlin:=xlin+FormatoMoneda(tot[i]-totDesc[i])+'//';
        BndDt.Items.Add(xlin);
      end;
    finally
      FAvance.Close;
...
procedure TFLIQREP06D.BndDtBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
...
//          AutoSize:=True;
        ...
        if xtp='TD' then begin
          Font.Style:=[fsBold];
          if (k>1) and not (k in [2,6,10,14])then begin
            Frame.DrawTop:=true;
          end;
        end;
      end;
...
procedure TFLIQREP06D.QRChildBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  PrintBand:=xtp='T';
end;

procedure TFLIQREP06D.QL_DliqcCalcFields(DataSet: TDataSet);
begin
  with DMGAS do begin
...
procedure TFLIQREP06D.QuickRep1BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
