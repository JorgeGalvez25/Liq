...
var
  FLIQREP29P: TFLIQREP29P;


implementation

uses ULIBGRAL, DDMGEN, UGEN_NET, ULIBDATABASE, ULIBPRINT,  DDMCFG, DDMGAS, UAVANCE,
     ULIQREP29, UGENCLIEB, DDMGENQ, DDMGEN_VTAS, DDMLIQ;

{$R *.DFM}

procedure TFLIQREP29P.FormShow(Sender: TObject);
begin
...
procedure TFLIQREP29P.BCancelarClick(Sender: TObject);
begin
  Close;
end;

procedure TFLIQREP29P.BImprimirClick(Sender: TObject);
      ...
      if sender=BImprimir then
        ...
        try
          with FLIQREP29 do begin
            ...
            Lista.Add('Clave'+ch+
                      'Nombre'+ch+
                      '1- '+IntToStr(VarCxcPlazo1)+' dias'+ch+
                       IntToStr(VarCxcPlazo1+1)+' - '+IntToStr(VarCxcPlazo2)+' dias'+ch+
                       IntToStr(VarCxcPlazo2+1)+' - '+IntToStr(VarCxcPlazo3)+' dias'+ch+
                       IntToStr(VarCxcPlazo3+1)+' o Mas'+ch+
                      'Saldo Actual'+ch);
...
procedure TFLIQREP29P.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if PrinterActiva then
    Printer.PrinterIndex:=PredPrinter1;
end;

procedure TFLIQREP29P.ComboBox1Change(Sender: TObject);
begin
  if PrinterActiva then
    Printer.PrinterIndex:=ComboBox1.ItemIndex;
end;

procedure TFLIQREP29P.MaskEdit1DblClick(Sender: TObject);
begin
...
procedure TFLIQREP29P.MaskEdit1Exit(Sender: TObject);
begin
...
procedure TFLIQREP29P.MaskEdit1Enter(Sender: TObject);
begin
...
procedure TFLIQREP29P.CheckBox2Click(Sender: TObject);
begin
...
procedure TFLIQREP29P.CheckBox1Click(Sender: TObject);
begin
  DBLookUpComboBox2.Enabled:=not CheckBox1.Checked;
end;

procedure TFLIQREP29P.DBLookupComboBox2Enter(Sender: TObject);
begin
