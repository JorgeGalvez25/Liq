...
uses DDMGEN, ULIBGRAL, UGEN_NET, ULIBDATABASE,DDMCUP,DDMGAS, DDMGASQ,
     DDMLIQ, ULIQREP46P, UAVANCE, DDMGENT;

{$R *.DFM}

procedure TFLIQREP46.CreaEtiquetas;
var i,xl,xw:integer;
begin
  xl:=xLeft;
...
procedure TFLIQREP46.IniciaEtiquetasCmb;
var i:word;
begin
  xl:=xLeft;
  for i:=1 to 10 do begin
    with QRLabelEnca[i] do begin
      Case i of
        ...
        4:Caption:='Importe';
        5:Caption:='Subtot A/IEPS';
        6:Caption:='Descuento';
        7:Caption:='Subtot C/Desc';
        8:Caption:='Costo';
        9:Caption:='Utilidad';
        10:Caption:='%';
      end;
...
procedure TFLIQREP46.IniciaEtiquetasProd;
var i:word;
begin
  xl:=xleft;
  for i:=1 to 10 do begin
...
procedure TFLIQREP46.PreparaReporte;
var TotCmb,TotAlma,TotPago,totclas:Array[1..9] of real;
    i,estacion,almacen:word;
    Precio,SubTotal,Desc,
    Costo,PorcUtil,Utilidad,
    Cantidad,Importe,
    Iva,Ajuste,Pago,
    PrecioReal, IEPS, IEPSLts,ImporteAIEPS:real;
begin
  with DMGEN,DMGENT,DMCUP,DMLIQ,DMGAS,FLIQREP46P do begin
    ModoRep:=xModo;
    CreaEtiquetas;
    IniciaEtiquetasCmb;
    ...
    while not QL_Dliqc.Eof do begin
      ...
      Q_AuxiReal1.FieldKind:=fkInternalCalc;
      Q_Auxi.SQL.Clear;
      Q_Auxi.SQL.Add('Select coalesce(Sum(Descuento),0) as Real1 from Dgasdescd');
      Q_Auxi.SQL.Add('Where Estacion='+IntToStr(estacion));
      Q_Auxi.SQL.Add('  and Combustible='+IntToStr(QL_DliqcCombustible.AsInteger));
      Q_Auxi.SQL.Add('  and Fecha>="'+FechaToStrSQL(VgFechaIni)+'"');
      Q_Auxi.SQL.Add('  and Fecha<="'+FechaToStrSQL(VgFechaFin)+'"');
      Q_Auxi.Open;
      Desc:=Q_AuxiReal1.AsFloat;

      Q_Auxi.Close;
      Q_AuxiReal1.FieldKind:=fkInternalCalc;
      Q_Auxi.SQL.Clear;
      if VarGasPreciosPorEstacion='Si' then begin
        Q_Auxi.SQL.Add('SELECT TASAIVA AS REAL1 FROM DGASPRECEST WHERE COMBUSTIBLE=1');
        Q_Auxi.SQL.Add(' AND FECHA=(SELECT MAX(FECHA) FROM DGASPRECEST WHERE COMBUSTIBLE=1)');
      end
      else begin
        Q_Auxi.SQL.Add('SELECT TASAIVA AS REAL1 FROM DGASPREC WHERE COMBUSTIBLE=1');
        Q_Auxi.SQL.Add('AND FECHA=(SELECT MAX(FECHA) FROM DGASPREC WHERE COMBUSTIBLE=1)');
      end;
      Q_Auxi.Open;
      Desc:=Desc/(1+(Q_AuxiReal1.AsFloat/100));
      ...
      TotCmb[5]:=TotCmb[5]+Desc;
      TotCmb[6]:=TotCmb[6]+(SubTotal-Desc);
...
//        end;
    ...
    BndDt.Items.Add('');
//
    SubTotal:=(TotCmb[3]-TotCmb[5])+TotAlma[3];
    BndDt.Items.Add('S//Resumen de Ventas:');
    BndDt.Items.Add('D//Combustibles// //'+FormatoMoneda(TotCmb[3]-TotCmb[5])+'//'+FormatFloat('##0.00',AjustaFloat(DivideFloat(TotCmb[3]-TotCmb[5],SubTotal)*100,2))+'%');
    BndDt.Items.Add('D//Productos// //'+FormatoMoneda(TotAlma[3])+'//'+FormatFloat('##0.00',AjustaFloat(DivideFloat(TotAlma[3],SubTotal)*100,2))+'%');
    BndDt.Items.Add('R// // //'+FormatoMoneda(TotCmb[3]+TotAlma[3]-TotCmb[5])+'//'+FormatFloat('#00.00',100.00)+'%');
//
    ...
    while not QL_Dliqp.Eof do begin
      ...
      if QL_DliqpClasePago.AsString='Efectivo' then
      Pago:=Pago-TotCmb[5];
...
procedure TFLIQREP46.QuickRep1BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  with DMGEN,DMGASQ do begin
    ...
    QRLEstacion.Caption:=FLIQREP46P.DBLookupComboBox2.Text;
...
procedure TFLIQREP46.BndDtBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
  ...
  begin
    for k:=ri to rf do
      with QRLabelDeta[k] do begin
        ...
        if xtp='T' then begin
          Font.Style:=[fsBold];
          if k in [2,4..10] then Frame.DrawTop:=true
...
procedure TFLIQREP46.ChildBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  PrintBand:=xtp='T';
end;

procedure TFLIQREP46.CreaTablaTemp;
begin
  with DMGEN do begin
...
procedure TFLIQREP46.TT_AuxiCalcFields(DataSet: TDataSet);
begin
