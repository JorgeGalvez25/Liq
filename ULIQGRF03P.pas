...
var
  FLIQGRF03P: TFLIQGRF03P;


implementation

uses ULIBGRAL, DDMGEN, UGEN_NET, ULIBDATABASE, ULIBPRINT, DDMGAS,
  ULIQDESGLO, DDMGASP, VarCmplx, DDMCONS, ULIQGRF03, DDMLIQ;

{$R *.DFM}

procedure TFLIQGRF03P.FormShow(Sender: TObject);
var
  xMes:string;
begin
...
//      if (UsuarioActivoLib = IdSuper) then
...
procedure TFLIQGRF03P.BCancelarClick(Sender: TObject);
begin
  Close;
end;

procedure TFLIQGRF03P.MaskEdit4KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=idF2 then
...
procedure TFLIQGRF03P.MaskEdit3Exit(Sender: TObject);
begin
  MaskEdit4.Text:=DateToStr(StrToDate(MaskEdit3.Text)+1);
end;

procedure TFLIQGRF03P.BPreverClick(Sender: TObject);
begin
...
procedure TFLIQGRF03P.CBTodasClick(Sender: TObject);
begin
  DBLookupComboBox2.Enabled := not CBTodas.Checked;
end;

procedure TFLIQGRF03P.DBLookupComboBox2Enter(Sender: TObject);
begin
...
procedure TFLIQGRF03P.CBTodasEnter(Sender: TObject);
begin
  if not DMLIQ.ValidaDerechoReportesConsultas(idLiq) then
    MaskEdit3.SetFocus;
end;

procedure TFLIQGRF03P.ActivarDesac(Periodo: byte);
begin
...
procedure TFLIQGRF03P.RGPeriodoClick(Sender: TObject);
begin
...
procedure TFLIQGRF03P.PreparaForma(xModo: Integer;xModuloAuxiliar:Boolean=False);
begin
  swModo:=xModo;
