...
procedure TFLIQREP22P.PreparaForma(xModo:integer);
begin
  ModoRep:=xModo;
end;

procedure TFLIQREP22P.BOpcionesClick(Sender: TObject);
begin
  PrintDialog1.Execute;
end;

procedure TFLIQREP22P.FormShow(Sender: TObject);
begin
...
procedure TFLIQREP22P.BCancelarClick(Sender: TObject);
begin
  Close;
end;

procedure TFLIQREP22P.BImprimirClick(Sender: TObject);
begin
  with DMGAS,DMGEN,DMCFG do begin
    ...
    Application.CreateForm(TFLIQREP22,FLIQREP22);
    try
      FLIQREP22.pEstacion:=T_EstsClave.AsInteger;
      FLIQREP22.PreparaReporte;
      if sender=BImprimir then
        FLIQREP22.QuickRep1.Print
      else
        FLIQREP22.QuickRep1.Preview;
    finally
      FLIQREP22.Free;
    end;
  end;
...
procedure TFLIQREP22P.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if PrinterActiva then
    Printer.PrinterIndex:=PredPrinter1;
end;

procedure TFLIQREP22P.ComboBox1Change(Sender: TObject);
begin
  if PrinterActiva then
    Printer.PrinterIndex:=ComboBox1.ItemIndex;
end;

procedure TFLIQREP22P.MaskEdit3Exit(Sender: TObject);
begin
  VgFechaIni:=StrToDate(MaskEdit3.text);
