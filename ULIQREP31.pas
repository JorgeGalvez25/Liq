...
type
  TFLIQREP31 = class(TForm)
    ...
    QL_DliqcJefeTurno: TIntegerField;
    TM_Auxi2: TRxMemoryData;
    TM_Auxi2Clave: TIntegerField;
    TM_Auxi2NombreJefe: TStringField;
    QL_Desp: TQuery;
    QL_DespCLAVE: TIntegerField;
    QL_DespNOMBRE: TStringField;
    QL_DespCTACNT: TStringField;
    QL_DespESTACION: TIntegerField;
    QL_DespACTIVO: TStringField;
    TM_Auxi2Fecha: TDateTimeField;
    TM_Auxi2Turno: TIntegerField;
    TM_Auxi2Caja: TIntegerField;
    QL_DliqcCaja: TIntegerField;
    TM_Auxi2Estacion: TIntegerField;
...
procedure TFLIQREP31.CreaEtiquetas;
var i,xl,xw:integer;
begin
  xw:=xAncho;
  xl:=xLeft;
...
procedure TFLIQREP31.PreparaReporte;
var tot:Array[1..8] of real;
    ...
    lts1,lts2,lts3,ltst,totJef:real;
...
begin
  with DMGEN,DMLIQ,DMGAS do begin
    ...
    QL_Dliqc.Close;
    QL_Dliqc.SQL.Clear;
    QL_Dliqc.SQL.Add('Select Estacion,Isla,Turno,Fecha,Folio,Combustible,Cantidad,Precio_Civa, ');
    QL_Dliqc.SQL.Add('  (Select Despachador from DGASLIQG where Estacion=DGASDLIQC.Estacion and Folio=DGASDLIQC.folio) as Desp');
    QL_Dliqc.SQL.Add(' from dgasdliqc');
    //QL_Dliqc.SQL.Add('Where Estacion=:pestacion and Fecha>=:pfechaini and Fecha<=:pfechafin');
    QL_Dliqc.SQL.Add('Where Fecha>=:pfechaini and Fecha<=:pfechafin');
    ...
    if (FLIQREP31P.ListadoEstaciones<>'*')and(LimpiaStr(FLIQREP31P.ListadoEstaciones)<>'') then
      QL_Dliqc.SQL.Add(' and Estacion in ('+FLIQREP31P.ListadoEstaciones+')');
...
//    QL_Dliqc.ParamByName('pestacion').AsInteger:=T_EstsClave.AsInteger;
    ...
    try
      ...
      while not QL_Dliqc.Eof do begin
        ...
        if not TM_Auxi2.Locate('Clave;Fecha;Turno;Caja',VarArrayOf([QL_DliqcJefeTurno.AsInteger,QL_DliqcFECHA.AsDateTime,QL_DliqcTURNO.AsInteger,QL_DliqcCaja.AsInteger]),[]) then begin
          TM_Auxi2.Insert;
          TM_Auxi2Clave.AsInteger:=QL_DliqcJefeTurno.AsInteger;
          if QL_Desp.Locate('Clave',QL_DliqcJefeTurno.AsInteger,[]) then
            TM_Auxi2NombreJefe.AsString:=QL_DespNOMBRE.AsString
          else
            TM_Auxi2NombreJefe.AsString:='Jefe de Turno no asignado.';
          TM_Auxi2Fecha.AsDateTime:=QL_DliqcFECHA.AsDateTime;
          TM_Auxi2Turno.AsInteger:=QL_DliqcTURNO.AsInteger;
          TM_Auxi2Caja.AsInteger:=QL_DliqcCaja.AsInteger;
          TM_Auxi2Estacion.AsInteger:=QL_DliqcESTACION.AsInteger;
          TM_Auxi2.Post;
        end;
        TT_Auxi.Edit;
        Case QL_DliqcCombustible.AsInteger of
          1:begin
              ...
              TotImp[1]:=TotImp[1]+AjustaFloat(QL_DliqcCantidad.AsFloat*QL_DliqcPrecio_Civa.AsFloat,2);
            end;
          2:begin
              ...
              TotImp[2]:=TotImp[2]+AjustaFloat(QL_DliqcCantidad.AsFloat*QL_DliqcPrecio_Civa.AsFloat,2);
            end;
          3:begin
              ...
              TotImp[3]:=TotImp[3]+AjustaFloat(QL_DliqcCantidad.AsFloat*QL_DliqcPrecio_Civa.AsFloat,2);
            end;
        end;
        TT_Auxi.Post;
      ...
      if ModoRep=0 then begin
        for i:=1 to 3 do begin
          ...
          Q_Auxi.SQL.Add('Where Combustible='+inttostr(i));
          //Q_Auxi.SQL.Add('where Estacion='+inttostr(T_EstsClave.AsInteger));
          if (FLIQREP31P.ListadoEstaciones<>'*')and(LimpiaStr(FLIQREP31P.ListadoEstaciones)<>'') then
            Q_Auxi.SQL.Add('and Estacion in('+FLIQREP31P.ListadoEstaciones+')');
          ...
          Q_Auxi.Close;
          _FactorAjusteReal:=DeterminaFactorDeAjusteMultiEsts(FLIQREP31P.ListadoEstaciones,VgFechaIni,VgFechaFin,i,pIsla,pTurno);
    ...
    try
      ...
      while not TT_Auxi.Eof do begin
        ...
        if ModoRep=0 then begin
          ...
          lts1:=ajustafloat(lts1-ajults[1]*factor,2);
          imp1:=ajustafloat(imp1-ajuimp[1]*factor,2);
          factor:=dividefloat(imp2,totimp[2]);
          lts2:=ajustafloat(lts2-ajults[2]*factor,2);
          imp2:=ajustafloat(imp2-ajuimp[2]*factor,2);
          factor:=dividefloat(imp3,totimp[3]);
          lts3:=ajustafloat(lts3-ajults[3]*factor,2);
          imp3:=ajustafloat(imp3-ajuimp[3]*factor,2);
        end;
        ltst:=lts1+lts2+lts3;
        impt:=imp1+imp2+imp3;
      ...
      //Jefes de turno
      BndDt.Items.Add('');
      BndDt.Items.Add('R//Jefes de Turno.//Fecha//Turno//Vtas. Aceites');
      TM_Auxi2.SortOnFields('clave;fecha');
      TM_Auxi2.First;
      while not TM_Auxi2.Eof do begin
        Q_Auxi.close;
        Q_Auxi.SQL.Clear;
        Q_AuxiReal1.FieldKind:=fkInternalCalc;
        Q_Auxi.SQL.Add('select coalesce(sum(d.importe),0) as Real1 from dgasdliqa d,dgasliqg m');
        Q_Auxi.SQL.Add('Where m.estacion=d.estacion');
        Q_Auxi.SQL.Add('  and m.folio=d.folio');
        Q_Auxi.SQL.Add('  and m.estacion='+TM_Auxi2Estacion.AsString);
        Q_Auxi.SQL.Add('  and m.caja='+TM_Auxi2Caja.AsString);
        Q_Auxi.SQL.Add('  and m.fecha='+QuotedStr(FechaToStrSQL(TM_Auxi2Fecha.AsDateTime)));
        if pIsla>0  then Q_Auxi.SQL.Add(' and d.Isla='+InttoStr(pIsla));
        if pTurno>0 then Q_Auxi.SQL.Add(' and m.Turno='+InttoStr(pTurno));
        Q_Auxi.SQL.Add('  and m.aplicado='+QuotedStr('Si'));
        Q_Auxi.Prepare;
        Q_Auxi.Open;
        xlin:='D//'+IntToClaveNum(TM_Auxi2Clave.AsInteger,3)+' '+TM_Auxi2NombreJefe.AsString+'//'+FechaPaq(TM_Auxi2Fecha.AsDateTime)+'//'+IntToClaveNum(TM_Auxi2Turno.AsInteger,3)+'//'+FormatoMoneda(Q_AuxiReal1.AsFloat);
        BndDt.Items.Add(xlin);
        totJef:=totJef+Q_AuxiReal1.AsFloat;
        TM_Auxi2.Next;
      end;
      xlin:='F// // //Total//'+FormatoMoneda(totJef);
      BndDt.Items.Add(xlin);
      BndDt.Items.Add('');
      BndDt.Items.Add('R//Resumen de Ventas');
      xlin:='D//Total Combustibles//'+FormatoMoneda(tot[8]);
      BndDt.Items.Add(xlin);
      xlin:='D//Total Aceites//'+FormatoMoneda(totJef);
      BndDt.Items.Add(xlin);
      xlin:='S//Total General//'+FormatoMoneda(tot[8]+totJef);
      BndDt.Items.Add(xlin);
...
procedure TFLIQREP31.QuickRep1BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
...
procedure TFLIQREP31.BndDtBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
...
procedure TFLIQREP31.ChildBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  PrintBand:=xtp='T';
end;

procedure TFLIQREP31.CreaTablaTemp;
begin
  with DMGEN do begin
...
procedure TFLIQREP31.FormDestroy(Sender: TObject);
begin
  with DMGEN do begin
    QL_Dliqc.Close;
    QL_Tcmb.Close;
...
procedure TFLIQREP31.TT_AuxiCalcFields(DataSet: TDataSet);
begin
...
procedure TFLIQREP31.QL_DliqcCalcFields(DataSet: TDataSet);
begin
  with DMGAS do begin
    if not SwCalcFields then
      exit;
    try
      Q_Auxi.Close;
      Q_Auxi.SQL.Clear;
      Q_AuxiEntero1.FieldKind:=fkInternalCalc;
      Q_Auxi.SQL.Add('select t.jefe_turno as Entero1 from dgasliqg l, dgasturc t');
      Q_Auxi.SQL.Add('where l.estacion=t.estacion');
      Q_Auxi.SQL.Add('   and l.fecha=t.fecha');
      Q_Auxi.SQL.Add('   and l.turno=t.turno');
      Q_Auxi.SQL.Add('   and l.caja=t.caja');
      Q_Auxi.SQL.Add('   and l.estacion='+QL_DliqcESTACION.AsString);
      Q_Auxi.SQL.Add('   and l.folio='+QL_DliqcFOLIO.AsString);
      Q_Auxi.SQL.Add('   and l.fecha='+QuotedStr(FechaToStrSQL(QL_DliqcFECHA.AsDateTime)));
      Q_Auxi.SQL.Add('   and l.turno='+QL_DliqcTURNO.AsString);
      Q_Auxi.Prepare;
      Q_Auxi.Open;
      QL_DliqcJefeTurno.AsInteger:=Q_AuxiEntero1.AsInteger;
    finally
      Q_Auxi.Close;
    end;
    try
      ...
      Q_Auxi.SQL.Clear;
      Q_AuxiEntero1.FieldKind:=fkInternalCalc;
      Q_Auxi.SQL.Add('select caja as Entero1 from dgasliqg');
      Q_Auxi.SQL.Add('where estacion='+QL_DliqcESTACION.AsString);
      Q_Auxi.SQL.Add('   and fecha='+QuotedStr(FechaToStrSQL(QL_DliqcFECHA.AsDateTime)));
      Q_Auxi.SQL.Add('   and turno='+QL_DliqcTURNO.AsString);
      Q_Auxi.SQL.Add('   and Folio='+QL_DliqcFOLIO.AsString);
      Q_Auxi.Prepare;
      Q_Auxi.Open;
      QL_DliqcCaja.AsInteger:=Q_AuxiEntero1.AsInteger;
    finally
      Q_Auxi.Close;
    end;
  end;
