...
var
  FLIQGRF05P: TFLIQGRF05P;


implementation

uses ULIBGRAL, DDMGEN, UGEN_NET, ULIBDATABASE, ULIBPRINT, DDMGAS,
  ULIQDESGLO, DDMGASP, VarCmplx, DDMCONS, ULIQGRF02, DDMLIQ, ULIQGRF05;

{$R *.DFM}

procedure TFLIQGRF05P.FormShow(Sender: TObject);
var
  xMes:string;
begin
...
procedure TFLIQGRF05P.BCancelarClick(Sender: TObject);
begin
  Close;
end;

procedure TFLIQGRF05P.MaskEdit4KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key=idF2 then
...
procedure TFLIQGRF05P.MaskEdit3Exit(Sender: TObject);
begin
  MaskEdit4.Text:=DateToStr(StrToDate(MaskEdit3.Text)+1);
end;

procedure TFLIQGRF05P.BPreverClick(Sender: TObject);
begin
...
procedure TFLIQGRF05P.CBTodasEnter(Sender: TObject);
begin
  if not DMLIQ.ValidaDerechoReportesConsultas(idLiq) then
    MaskEdit3.SetFocus;
end;

procedure TFLIQGRF05P.ActivarDesac(Periodo: byte);
begin
...
procedure TFLIQGRF05P.RGPeriodoClick(Sender: TObject);
begin
...
procedure TFLIQGRF05P.PreparaForma(xModo: Integer;xModuloAuxiliar:Boolean=False);
begin
  swModo:=xModo;
