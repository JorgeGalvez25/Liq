...
var
  FLIQGRF10: TFLIQGRF10;

implementation

uses ULIQGRF10P, ULIBGRAL, ULIBDATABASE, Printers, DDMGAS, UAVANCE;

{$R *.dfm}

procedure TFLIQGRF10.PreparaForma(xModo:Integer; swAnio:string);
var
  xDias, xProm:Integer;
  xLtsTots1,xLtsTots2:Real;
...
begin
  ...
  if xModo=1 then
    Chart1.Title.Text.Add('Comparativo de  ventas en litros de combustible por estación*')
  else
    Chart1.Title.Text.Add('Comparativo de  ventas en litros de combustible por estación');
  if not FLIQGRF10P.GBMes.Visible then begin
  ...
  with FLIQGRF10P, DMGAS do begin
    ...
    dmgas.Q_Auxi.SQL.Add('Select sum(c.Cantidad) as Real1,');
    if GBFecha.Visible then begin
      ...
      QL_Ajus.Close;
      QL_Ajus.SQL.Clear;
      QL_Ajus.SQL.Add('select sum(coalesce(Diferencia,0)) as Diferencia,');
      ...
      QL_AjusA.Close;
      QL_AjusA.SQL.Clear;
      QL_AjusA.SQL.Add('select sum(coalesce(Diferencia,0)) as Diferencia,');
...
//              Q_Auxi2.SQL.Add('  and estacion='+IntToStr(estacion));
        ...
        if TM_Fecha2.Locate('Fecha',Q_AuxiDate1.AsDateTime,[]) then begin
          TM_Fecha2.Edit;
//          TM_Fecha2Indice.AsInteger:=i;
...
//          TM_Fecha2.Post;
    ...
    Chart1.Title.Text.Add('Promedio '+xAnio1+': '+FormatoMoneda(DivideFloat(xLtsTots1,xProm)));
    Chart1.Title.Text.Add('Promedio '+xAnio2+': '+FormatoMoneda(DivideFloat(xLtsTots2,xProm)));
  end;
end;

procedure TFLIQGRF10.ActionSalirExecute(Sender: TObject);
begin
  Close;
end;

procedure TFLIQGRF10.DesactivarCantidades1Click(Sender: TObject);
begin
  Series1.Marks.Visible := not Series1.Marks.Visible;
  Series2.Marks.Visible := not Series2.Marks.Visible;
end;

procedure TFLIQGRF10.PopupMenuDesCantPopup(Sender: TObject);
begin
...
procedure TFLIQGRF10.BitBtn1Click(Sender: TObject);
begin
