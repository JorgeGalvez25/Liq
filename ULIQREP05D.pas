...
type
  TFLIQREP05D = class(TForm)
    ...
    QL_Descd: TQuery;
    QL_DescdCOMBUSTIBLE: TIntegerField;
    QL_DescdDESCUENTO: TFloatField;
...
uses DDMGEN, ULIBGRAL, UGEN_NET, ULIBDATABASE,DDMCUP,DDMGAS, DDMGASQ,
     DDMLIQ, ULIQREP05P, ULIQDESGLO, DDMGENT, UAVANCE, DDMCONS, StrUtils;

{$R *.DFM}

procedure TFLIQREP05D.CreaEtiquetas;
var i,xl,xw:integer;
begin
  xl:=xLeft;
  for i:=1 to 10 do begin
    ...
    with QRLabelEnca[i] do begin
      Case i of
        ...
        8:Caption:='';
        9:Caption:='I.V.A.';
        10:Caption:='Total Neto';
      end;
      if i=1 then xw:=120;
      if i in [2,8] then xw:=0;
      if (i in [3..7]) or (i in [9,10]) then xw:=81;
      Autosize  :=false;
      Left      :=xl;
      Width     :=xw;
      Top       :=2;
      ...
      if i in [1,2,8,10] then Frame.DrawRight:=false
...
procedure TFLIQREP05D.IniciaEtiquetasCmb;
var i:word;
begin
  xl:=xLeft;
  for i:=1 to 10 do begin
...
procedure TFLIQREP05D.IniciaEtiquetasProd;
var i:word;
begin
  xl:=xleft;
  for i:=1 to 10 do begin
    with QRLabelEnca[i] do begin
      ...
      if i in [9,10] then Frame.DrawRight:=False;
      if i in [2..7] then xw:=65;
      if i in [8] then xw:=80;
      if i in [9,10] then xw:=0;
      Left      :=xl;
      Width     :=xw;
    end;
    with QRLabelDeta[i] do begin
...
procedure TFLIQREP05D.PreparaReporte(xModo:integer);
var totc,tota,tota2,subtota2,
    totp,totcant,precio,totd,
    precioprom,venta,
    subtotal,iva,subaux,
    subtotalDescto,ivaDescto,
    subtotalCDescto,ivaCDescto,
    tsubtotal,tiva,
    tsubtotalCDescto,tivaCDescto,
    ttsubtotalCDescto,ttivaCDescto,
...
begin
  with DMGEN,DMGENT,DMLIQ,DMCUP,DMGAS,FLIQREP05P do begin
    ...
    CreaEtiquetas;
    totc:=0; totcant:=0; ttimp:=0; ttimpdevP:=0; ttimpdevC:=0; totd:=0;
    ...
    QL_Descd.Close;
    QL_Descd.SQL.Clear;
    QL_Descd.SQL.Add('select');
    QL_Descd.SQL.Add('  combustible,');
    QL_Descd.SQL.Add('sum(descuento) as DESCUENTO');
    QL_Descd.SQL.Add('from');
    QL_Descd.SQL.Add('dgasdescd');
    if CBEst.Checked then
      QL_Descd.SQL.Add('Where ' )
    else
      QL_Descd.SQL.Add('Where Estacion='+IntToStr(estacion)+' and ' );
    QL_Descd.SQL.Add('  Fecha>='+QuotedStr(FechaToStrSQL(VgFechaIni))+' and Fecha<='+QuotedStr(FechaToStrSQL(VgFechaFin)));
    QL_Descd.SQL.Add('group by combustible');
    QL_Descd.Prepare;
    QL_Descd.Open;
...
//        else
          ...
          Subtotal:=AjustaFloat(DivideFloat(Importe,1+GetTasaIVAdeCombustible(estacion,QL_DliqcCOMBUSTIBLE.AsInteger,VgFechaFin)/100),2); //primero se le tiene que quitar el EIPS
        ...
        if LocalizaQuery(QL_Descd,'COMBUSTIBLE',QL_DliqcCOMBUSTIBLE.AsInteger) then
          descuento:=QL_DescdDESCUENTO.AsFloat;
        ...
        if not SwDesglose then begin
          if descuento>0 then begin
            subtotalDescto:= (QL_DescdDESCUENTO.AsFloat)/(1+GetTasaIVAdeCombustible(estacion,QL_DliqcCOMBUSTIBLE.AsInteger,VgFechaFin)/100);
            ivaDescto:= (QL_DescdDESCUENTO.AsFloat)-(subtotalDescto);
          end
          else begin
            subtotalDescto:=0;
            ivaDescto:=0;
          ...
          subtotalCDescto:=subtotal-subtotalDescto;
          ivaCDescto:=iva-ivaDescto;
          ...
          xlin:='D//'+QL_DliqcNombreCombustible.AsString+'// //'+FormatFloat('##,##0.0000',Cantidad)+'//'+
                      ...
                      FormatoMoneda(subtotalCDescto)+
                      '// //'+
                      FormatoMoneda(ivaCDescto)+'//'+
                      FormatoMoneda((subtotal+iva)-descuento);
          tsubtotalCDescto:=tsubtotalCDescto+subtotalCDescto;
          tivaCDescto:=tivaCDescto+ivaCDescto;
        end
        else begin
          ...
          xlin:='D//'+QL_DliqcNombreCombustible.AsString+'// //'+FormatFloat('##,##0.0000',Cantidad)+'//'+
                      ...
                      FormatoMoneda(ivaCDescto)+'//'+
                      FormatoMoneda((subtotal+iva)-descuento);
          ttsubtotalCDescto:=ttsubtotalCDescto+subtotalDescto;
          ttivaCDescto:=ttivaCDescto+ivaDescto;
        end;

        totalesIVA[indice]:=ivaCDescto;
        indice:=indice+1;
    ...
    if not SwDesglose then
      xlin:='T// // //'+FormatoMoneda(totcant)+'// //'+FormatoMoneda(totc)+'//'+
                        FormatoMoneda(totd)+'//'+FormatoMoneda(tsubtotalCDescto)+'// //'+
                        FormatoMoneda(tivaCDescto)+'//'+FormatoMoneda(totc-totd)
    else
      xlin:='T// // //'+FormatoMoneda(ttotc)+'// //'+FormatoMoneda(ttimp)+'//'+
                        FormatoMoneda(totd)+'//'+FormatoMoneda(ttsubtotalCDescto)+'// //'+
                        FormatoMoneda(ttivaCDescto)+'//'+FormatoMoneda(ttimp-totd);
    BndDt.Items.Add(xlin);
...
//          DMCUP.DamePrecioFecha(TM_AuxiComb.AsInteger,VgFechaFin,precioprom,xTasaIva);
        ...
        FAVANCE.Close;
        xlin:='F// // //'+FormatoMoneda(totcantFct)+'// //'+FormatoMoneda(totcFct)+'// //'+
                      FormatoMoneda(tsubtotalFct)+'// //'+
                      FormatoMoneda(tivaFct)+'// //';
        BndDt.Items.Add(xlin);
        
        BndDt.Items.Add('D//');

        xlin:='F//Total Combustible // //'+FormatoMoneda(totcant+totcantFct)+'// //'+FormatoMoneda(totc+totcFct)+'// //'+
                        FormatoMoneda(tsubtotal+tsubtotalFct)+'// //'+FormatoMoneda(tiva+tivaFct)+'//';
        BndDt.Items.Add(xlin);
      end;
    end;
...
//
    ...
    if SwDesglose then begin
      ttimp:=ttimp-totd;
      BndDt.Items.Add('D//Combustibles'+IfThen(totd>0,' (con descuento)','')+'// //'+FormatoMoneda(ttimp)+'//'+
      ...
      totc:=totc-totd;
      BndDt.Items.Add('D//Combustibles'+IfThen(totd>0,' (con descuento)','')+'// //'+FormatoMoneda(totc+totcFct)+'//'+
                          FormatFloat('##0.00',AjustaFloat(DivideFloat(totc+totcFct,totc+totcFct+tota2)*100,2))+'%');
      BndDt.Items.Add('D//Productos// //'+FormatoMoneda(tota2)+'//'+
                          FormatFloat('##0.00',AjustaFloat(DivideFloat(tota2,totc+totcFct+tota2)*100,2))+'%');
      BndDt.Items.Add('R// // //'+FormatoMoneda(totc+totcFct+tota2)+'//100.00%');
    end;
//
...
//    QL_Dliqp.ParamByName('pfechafin').AsDateTime:=VgFechaFin;
    ...
    try
      ...
      while not QL_Dliqp.Eof do begin
        ...
        if (QL_DliqpClasePago.AsString='Efectivo')and(not swTPagEfectivoConDescuento) then
          Pago:=Pago-totd;
...
//        else
  //        xlin:='D//'+QL_DliqpDescripcionPago.AsString+'// //'+FormatoMoneda(Pago);
          xlin:='D//'+QL_DliqpDescripcionPago.AsString+IfThen((totd>0)and(QL_DliqpClasePago.AsString='Efectivo')and(not swTPagEfectivoConDescuento),' (con descuento)','')+'// //'+FormatoMoneda(Pago)+'//'+
                      FormatFloat('##0.00',AjustaFloat(DivideFloat(Pago,totc+totcFct+tota2)*100,2))+'%';
    ...
    if CBIEPS.Checked then begin
      ...
      try
        ...
        if not SwDesglose then begin
          BndDt.Items.Add('X// // //Total... // '+FormatoMoneda(IEPSLts)+' //'+FormatoMoneda(tivaCDescto));
        end
        else begin
          BndDt.Items.Add('X// // //Total... // '+FormatoMoneda(IEPSLts)+' //'+FormatoMoneda(ttivaCDescto));
        end;
      finally
        FAVANCE.Close;
      end;
...
procedure TFLIQREP05D.QuickRep1BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
...
procedure TFLIQREP05D.BndDtBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
  ...
  begin
    for k:=ri to rf do
      with QRLabelDeta[k] do begin
        ...
        if (xtp='D')and(xtipo=1) then begin
          if k=1 then Width:=120;
          if k=3 then Width:=81;
        end;
        ...
        if xtp in ['A','E'] then begin
          if xtp ='E' then begin
            if (k=1)and(xtipo=1) then Width:=200;
            if (k=3)and(xtipo=1) then Width:=0;
          end;
          Font.Style:=[fsBold];
        end;
        if xtp='T' then begin
          ...
          if xtipo=1 then begin
            if k in [3,5,6,7,9,10] then Frame.DrawTop:=true
        ...
        if xtp='F' then begin
          if not pSwNegritas then Font.Style:=[fsBold];
          if k in [3,5,7,9] then Frame.DrawTop:=true
          else Frame.DrawTop:=false;
        end;
      end;
  end;
...
begin
  ...
  InicializaEtiquetas(1,10);
  ExtraeEtiquetas(1,10);
end;

procedure TFLIQREP05D.ChildBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  PrintBand:=xtp='T';
end;

function TFLIQREP05D.medicion_tanque_turno_anterior(tanque: string; fecha_completa: TDateTime): real;
var
  FechaAnt:TDateTime;
  CorteAnt:Integer;
begin
...
function TFLIQREP05D.Medicion_tanque_UltimoTurno(tanque:string;FechaIni,FechaFin:TDateTime):Real;
var
  FechaMax:TDateTime;
  CorteMax:Integer;
begin
...
function TFLIQREP05D.Entrada_Tanque_Fecha(tanque: string;
